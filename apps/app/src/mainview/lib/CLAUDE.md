...
| 2 | **Side effects inside selectors** | `selectChatMessages` (lines 1660–1675) fires `loadReplyThumb()` and `resolveReplyPreview()` during selection — mutating module-level Sets and triggering async fetches as a side effect of reading state. | Selectors must be **pure reads**. Side effects belong in an `useEffect` in the component that consumes the selector, or in a store action triggered after state settles. |
